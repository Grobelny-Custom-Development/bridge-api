...
class Command(BaseCommand):
    ...
    def generate_generic_meeting_templates(self):
        ...
        baseline_descriptions = [
            'Sprint Planning for groups',
            'Forced Ranking for groups',
            'Grouping for groups',
            'Bucketing for groups',
            'Prioritization for groups']
        baseline_component_names = [
            'Brainstorm',
            'Forced Rank',
            'Grouping',
            'Bucketing',
            'Prioritization']
        baseline_component_descriptions = [
        ...
        for host in hosts:
            selected_components = []
            for name, description, activity_choice in zip(
                    baseline_component_names, baseline_component_descriptions, ACTIVITY_CHOICES):
                selected_component = {
                    'name': name,
            ...
            for name, description in zip(
                    baseline_names, baseline_descriptions):
                create_meeting_template_components(
                    host,
                    name,
                    description,
                    recurring,
